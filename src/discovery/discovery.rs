...
use crate::discovery::discovery::server::GrpcServer;
use crate::permastate::EntityService;
use actix::prelude::*;
use log::info;
...
impl Handler<StartMessage> for ProtocolHandlerActor {
    ...
    fn handle(&mut self, _msg: StartMessage, _ctx: &mut Context<Self>) -> Self::Result {
        ...
        Ok(GrpcServer::new(_msg.opts).start().is_ok())
    }
}
...
pub mod server {
    ...
    use crate::discovery::discovery::Options;
    use log::{debug, info};
    use std::fs::File;
    use std::io::Read;
    ...
    use crate::discovery::discovery::spec::{
        entity_discovery_server::{EntityDiscovery, EntityDiscoveryServer},
        Entity, EntitySpec, ProxyInfo, ServiceInfo, UserFunctionError,
    };
    ...
    impl EntityDiscovery for Discover {
        async fn discover(
            ...
            debug!(
                "Received discovery call from sidecar with Metadata [{:?}",
                metadata
            );
            info!(
                "Received discovery call from sidecar [{:?} {:?}] supporting CloudState {:?}.{:?}",
                proxy_info.proxy_name,
                proxy_info.proxy_version,
                proxy_info.protocol_major_version,
                proxy_info.protocol_minor_version
            );
            info!(
                "Supported sidecar entity types: {:?}",
                proxy_info.supported_entity_types
            );
            ...
            let lib_name: String = String::from("eigr-rust-support");
        ...
        async fn report_error(
            ...
            error!(
                "Received report_error from sidecar. Error: {:?}",
                user_func_error.message
            );
            Ok(Response::new(()))
        }
    }
    ...
    impl GrpcServer {
        ...
        pub fn start(self) -> Result<(), Box<dyn std::error::Error>> {
            ...
            rt.block_on(async {
                ...
                let addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(0, 0, 0, 0)), opts.server_port);
